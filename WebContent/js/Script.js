	...
	xhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
			...
			if ($.parseJSON(xhttp.responseText).length > 0) {
				...
				$("#test")
						.append(
								"<table class=\"table table-striped\"><thead><tr><th>Related Links:</th></tr></thead><tbody>"
	...
	xhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
			...
			if ($.parseJSON(xhttp.responseText).length > 0) {
				...
				$("#test")
						.append(
								"<table class=\"table table-striped\"><thead><tr><th>Did you mean :</th></tr></thead><tbody>"
										+ createTable + "</tbody></table>");
			} else {
				document.getElementById("solutionBar").style.display = "block";
				$("#test").replaceWith("<div id = \"test\"></div>");
				$("#test")
						.append(
								"<table class=\"table table-striped\"><thead><tr><th>Did you mean :</th></tr></thead></table>");
			}
		}
	};
	var str = $('#usr').val();
